...
> **Repository status:** accepted architecture, **Stages 0-2 complete**, **Stage 3 open** (`stage-3-persistence`). The design is under [`architecture/`](./architecture/). Read `[workspace].members` in [`Cargo.toml`](./Cargo.toml) for the active roster rather than a list written here; the `crates/api/*` contracts are types and validation only, no I/O and no backend. Stage 3 is where the first file I/O lands — the human store, the peer cache, and the profile path layout. `apps/`, `spikes/`, and `packaging/` are still empty of implementation. The next package joins when the stage that needs it opens.

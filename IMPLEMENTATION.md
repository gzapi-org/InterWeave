...
**Stage 0 is complete; Stage 1 is open.** The virtual root [`Cargo.toml`](./Cargo.toml) lists the active members; at the time of writing they are [`xtask`](./xtask/README.md), the command runner, `tests/support`, the test-only harness, and the neutral contracts [`transport-api`](./crates/api/transport-api/README.md), [`trust-api`](./crates/api/trust-api/README.md), and [`discovery-api`](./crates/api/discovery-api/README.md). The manifest is authoritative. `workspace.metadata.interweave` records the remaining planned member/test paths without making them buildable; when a canonical bottom-up stage starts, add a crate manifest only for the crate/package being implemented and add that path to `[workspace].members` in the same change.

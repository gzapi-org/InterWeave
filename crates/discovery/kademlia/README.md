...
**Current status:** Stage 10, active workspace member. Provider core: targeted-lookup eligibility (kademlia-integration.md §9.2, one conjunct at a time), query-result normalization with `"kademlia"` provenance and candidate TTL (§10), and health over the port's `RoutingView` (§14) — server mode reports at most degraded this stage, because the strong reachability evidence classes arrive with AutoNAT/Relay at Stage 11. The scheduler/budgets and the Swarm-owned driver in `crates/transport/libp2p` land later in this stage.

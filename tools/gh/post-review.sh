...
[[ -f "$target" ]] || {
    echo "post-review: agent-fabric not found at $fabric (expected beside this working copy, as projects/agent-fabric); set AGENT_FABRIC_ROOT or check it out. See CLAUDE.md - the control plane." >&2
    exit 2
}
exec bash "$target" "$@"

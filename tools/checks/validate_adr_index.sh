...
# written went stale and this script prints the real one at runtime; the
# replacement for it was itself wrong by one, which is the argument for
# having no number here at all -- so an ADR missing from it is invisible — the failure mode is

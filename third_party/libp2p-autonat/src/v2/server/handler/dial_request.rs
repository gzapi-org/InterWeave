...
async fn handle_request(
    ...
    // read before the response is consumed by the send. The variant
    // names are prost's since 0.16 -- `Ok`, `EDialError`,
    // `EDialBackError`, `Unused` -- where 0.15's quick-protobuf spelled
    // them `OK`, `E_DIAL_ERROR`, `E_DIAL_BACK_ERROR`, `UNUSED`; the wire
    // values and this mapping are unchanged.
    let dial_back = match response.dial_status {
        DialStatus::Ok => DialBackOutcome::Ok,
        DialStatus::EDialError => DialBackOutcome::DialError,
        DialStatus::EDialBackError => DialBackOutcome::DialBackError,

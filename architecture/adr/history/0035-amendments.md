...
The standard-v1 AutoNAT v2 client chooses its probe server itself, uniformly at random among the connections this profile DIALLED whose remote advertised the dial-request protocol, and its entire configuration surface is a candidate ceiling and a probe interval. There is no hook to rank servers, prefer a source, or veto a pick. Nor can the outbound dial gate stand in for one: an AutoNAT probe is a request over an already-open connection, so the client emits no dial for the gate to judge. What is left that a profile can decide is **which servers it DIALS**, and that is what static configuration and `use_authorized_identify_servers` govern for AutoNAT under this amendment. The verb matters: the client installs its dial-request handler only in `handle_established_outbound_connection`, so an implementer who satisfied "connects to" by accepting those servers' inbound connections would have no eligible probe servers at all.

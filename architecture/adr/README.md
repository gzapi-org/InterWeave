...
| [0011](./0011-discovery-connection-ownership.md) | Discovery owns candidates; ConnectionManager owns connection policy, enforced for explicit and behaviour-originated Swarm dials; a discovery provider never writes the Swarm's address book (A 2026-09-20) — through either door: the address emission is swallowed and the pending-dial hook answered with nothing (A 2026-09-25). |
...
| [0051](./0051-vendor-and-patch-the-autonat-v2-client.md) | Build `libp2p-autonat` 0.15.0 from a vendored copy carrying two recorded patches — `Behaviour::retest` with the declined outcome that makes it safe, and a dial status on the vendored server's public `Event` (the patch itself still owed) — because the released v2 client tests an address once and can never supply ADR-0035's second observer or AUTONAT.md §5's refresh. |
| [0052](./0052-peer-supplied-addresses-are-dialled-inside-an-address-class-boundary.md) | Every address this runtime dials because a peer supplied it — an AutoNAT dial-back, a DCUtR punch candidate, any future instance — sits inside an address-class boundary enforced at the wrapper's pending hook before any socket: literal IP only, no relayed address, loopback/link-local/special-use always refused, private ranges decided per instance (the punch admits them only where this node itself listens privately), one predicate family bound by a subset test, no test-only knob; a name resolves only when the operator supplied it, and every path that enters the address book — Identify's listen_addrs included — is an instance (A 2026-09-20); stores hook at their learn sites, dials are enforced once at a root funnel around the composed behaviour, and provenance is the door, never a tag — one operator set admits the operator's names; a NATed peer's own advertised circuit enters the book when its relay prefix passes the floor (A 2026-09-25). |
| [0053](./0053-vendor-libp2p-mdns-and-bound-the-stores-a-lan-feeds.md) | Build `libp2p-mdns` 0.49.0 from a vendored copy carrying one recorded diff — caps on the three stores the multicast domain feeds, the announcer's TTL clamped to the provider's, RFC 6762's once-per-second reply rule, and an additive interface-failure event — because the released crate bounds none of them and reports none of its failures; the second vendored crate under ADR-0051's route. |

...
- Rules: `third_party/libp2p-mdns/` via `[patch.crates-io]`, ADR-0051 Decisions 1, 2, 5–8 unchanged (D1). Caps on `discovered_nodes` in the provider's SHAPE (at most 256 peers and 8 addresses per peer, both drift-checked — a count-only 2048 let the crate hold records the provider refused), per-interface `discovered` and `send_buffer`; past a cap a record is dropped and counted (D2). The announcer's TTL is clamped to the provider's 120 s observation TTL, so a long-TTL flood cannot out-live legitimate records (D3). A given answer is multicast at most once per second per interface ADDRESS, RFC 6762 §6 (D4). Two additive events — `Event::InterfaceFailed` for bind/join, receive and send failures and `Event::WatcherFailed` for the interface watcher — surfaced by the runtime as `SwarmEvent::MdnsInterfaceFailed` / `MdnsWatcherFailed` and mapped to degraded by Stage 12's composer (the mapping owed with that stage); a watcher erroring on two consecutive polls is dead and no longer polled (built on PR #112); the behaviour rebuild after it runs on the refresh tick — at most one per `REFRESH_INTERVAL`, a failed one keeps the running behaviour and reports `MdnsRebuildFailed`, its own event (`MdnsUnavailable` is the start's), the swap re-tells every listen address — on top of an `impl Drop for Behaviour` that aborts the interface tasks, since the crate detaches them and `SO_REUSEPORT` let a rebuild add an answerer per interface (measured); landed with p2p-network-dev's PR #120, 53cce603; a held `MdnsRebuildFailed` is dropped when a later rebuild succeeds, and the drop-count cell keeps the last replaced behaviour's counters live until the next replace (A 2026-09-26) (D5, D7). Unsolicited responses stay accepted — the protocol (D6). Every drop counted on the runtime handle, no address logged (D7). Nothing else patched: the patch is rules 2–5, the log discipline, the stop with the `Provider` export, the accessor and the `Drop` impl (D8). Order: flood harness unpatched → verbatim vendor → patch → harness asserts; the mDNS deadline reads MET only then (D9). The crate's store is the liveness authority: the driver re-pushes its live records every 60 s through an additive accessor so the provider never forgets a live peer (D10); the accessor, that refresh, the `Drop` and the behaviour rebuild landed with p2p-network-dev's PR #120 (A 2026-09-26). The once-per-second bound is per answer across answers; a multipart peer answer repeats the PTR by design (D4); the send-buffer backstop is reached by local configuration at 465 relevant listen addresses (D9).
- Keywords: libp2p-mdns, vendored dependency, third_party, discovered_nodes, unbounded store, multicast flood, amplification, RFC 6762, announcer TTL, TTL clamp, InterfaceFailed, WatcherFailed, MdnsRebuildFailed, degraded signal, second vendored crate
...
- Discovery never writes the address book (Amendment 2026-09-20): a provider yields candidates; a transport behaviour's address emission (`libp2p-mdns 0.49` `NewExternalAddrOfPeer`, untrusted) is swallowed at the wrapper; the only path to a dialable address is normalization → `DiscoveryManager` → ConnectionManager admission; binds every provider.
- Identify's advertised addresses enter the book through ADR-0052's boundary (Amendment 2026-09-20, recorded 2026-09-25): a `listen_addr` is peer-supplied and an instance of ADR-0052 rule 8 — hooked at the LEARN site with the floor, rule 3 beside a same-family private listener, no rule 4; a refused address never becomes an entry. It is a STORE hook, not the dial's enforcement (Amendment 2026-09-25): the dial is ADR-0052 rule 5's root funnel, and `libp2p-identify`'s own address cache — a second store nobody owned — is disabled so the book is the only address store the runtime writes from Identify — the crates' own stores (Kademlia's table, request-response's `PeerAddresses`, the mDNS crate's records) are named with their doors.
- The Identify instance admits a peer's own circuit address (Amendment 2026-09-25): an advertised `/p2p-circuit` whose transport prefix meets ADR-0052's floor or is an operator address enters the book; the relay's admission is the gate's at dial time.
- Two doors, several stores (Amendment 2026-09-25): request-response 0.30.0 consumes `NewExternalAddrOfPeer`, so the swallow is enforcement; a wrapped behaviour's second door is its pending-dial hook answer, which `MdnsScope` now answers with nothing.
- Keywords: connectionmanager, dialadmissiongate, behaviour-originated dial, address quarantine, identity mismatch, poisoned address, backoff, address book, newexternaladdrofpeer, discovery never writes the book, identify listen_addrs, identify cache, store hook, advertised circuit, pending-dial hook, peeraddresses
...
- Reservations: 2 distinct relay PeerIds when direct is `Unknown`/`NotVerified`, 1 warm reservation when `VerifiedPublic`, maximum 4 active. Static configured relays by default; Identify-learned candidates require explicit opt-in and lose precedence to static ones — **precedence binds relay only** since the Amendment 2026-09-09, because the standard-v1 AutoNAT client can be given no server order, so for AutoNAT those settings decide which servers the profile DIALS (one that dialled us is not eligible). Relay discovery never uses Kademlia provider/value records.
...
The released client tests an address once; ADR-0035 needs two observers and §5 needs refresh, so the workspace builds a vendored copy with two recorded patches — the re-test method, and (A 2026-09-18, the patch itself still owed) a dial status on the server's public `Event`.
- Rules: `libp2p-autonat` 0.15.0 is built from `third_party/libp2p-autonat/` via `[patch.crates-io]`, the crates.io tarball **minus its packaging files** (`.cargo_vcs_info.json`, `Cargo.toml.orig`, `Cargo.lock`), plus the upstream `LICENSE` and `INTERWEAVE.patch` — Decision 2 is the authority on the list, and an earlier wording here said "the tarball plus ... and **nothing else**", which reads as keeping those three. The patch is two coupled halves in three hunks: `Behaviour::retest(&Multiaddr) -> bool` returns a tested candidate to the sweep, and the `AddressNotReachable` arm declines to emit for a probe `retest` abandoned. WHEN to re-test — refresh, a further observer, retry under the dial gate's constants — is the `ReachabilityManager`'s decision. **A path-patched crate is invisible to `cargo-deny` as well as to Dependabot** — measured, not assumed — so `tools/checks/check_vendored_advisories.sh` re-asks the advisory question against the registry at the vendored version, and is the only warning a vendored tree gets. Every vendored file is in `license_exempt.txt` with provenance and checksum. Dropped when a release carries an equivalent; every libp2p bump re-vendors and re-applies. Server choice stays the crate's random pick, so the second observer converges probabilistically and the manager counts distinct servers. The owed two-Swarm test has two halves: crate-level on loopback, written on PR #90 ahead of step 4 (the server dials back any requested address), substrate-level at SPIKE-004 phase B (amended 2026-09-18). **A 2026-09-18 (step 4 removed the four server look-alikes):** the Decision 4 paragraph and table that ordered `AutonatServerConfig::timeout_ms`, the schema/example `server.timeout` keys and the two specification rows to survive step 3's removal read as history — step 4 (PR #93, merge f21e8e75) removed all four, the server bounding request and dial-back at 10 s in code with no setter; `AUTONAT.md` §7 carries the dated note. **A 2026-09-18 (a second recorded patch):** `INTERWEAVE.patch` carries two patches — `retest`, and an additive dial-status field on the vendored server's public `Event`, because `served_ok` counted a delivered `E_DIAL_BACK_ERROR` as served (the outcome lived only on the crate-internal back channel); Decisions 2/3/3a/8; the patch itself is p2p-network-dev's recorded gap.
- Keywords: vendored dependency, third_party, patch.crates-io, retest, candidate lifecycle, second observer, refresh, dependabot blind spot, cargo-deny blind spot, vendored crate advisories, server event dial status, served_ok, second patch

### 0052 — Peer-supplied addresses are dialled inside an address-class boundary (Accepted)
A peer's trust authorizes its protocol, never where this host opens sockets: AutoNAT's dial-back (`AUTONAT.md` §7) had the boundary and DCUtR's punch candidates (`DCUTR.md` §6) had none, so the cross-cutting rule is recorded once with both as instances.
- Rules: floor for every instance — literal IP multiaddr, no DNS, no `p2p-circuit`; loopback, unspecified, multicast, link-local and special-use refused whoever supplies them; global admitted (D2). Private ranges per instance: the dial-back refuses them; the punch admits a private candidate only when this node holds a non-loopback listener in a private range of the same family — the LAN case (D3). Source-equality is the dial-back's alone; a punch candidate legitimately differs from the observed address (D4). Enforced at the wrapper's pending outbound hook before any socket, refusal counted by class, address never logged (D5). One predicate family beside `is_probeable_address`, a subset test binds the two lists (D6). No test-only knob; a refused class is shown REFUSED, the connection-made test runs at crate level or over a private pair (D7). A new peer-supplied dial states its instance in its transport document before its hook (D8). Lands on PR #102 with the DCUtR hook.
- Peer-supplied names and the paths that enter the book (Amendment 2026-09-20): rule 2's "no DNS name" is a RESOLVER POLICY — a name resolves only when the operator supplied it (configuration, the operator's `AddAddress`); a peer's name is refused at the boundary whatever the trust, and the DNS transport stays for the operator's names. Rule 8 covers every path that ENTERS the book or a dial, not only a new dial, with the learn site as the hook where no dial originates (mDNS's shape); Identify `listen_addrs` is an instance that was inside rule 1 from the start with no hook (PR #111 lands it beside the DNS transport); Kademlia records and PeerCache re-entry are named as paths to be read.
- Dials are enforced once, at the root of the outbound path (Amendment 2026-09-25): a dial that EXTENDS its addresses through the behaviours (Kademlia's walk, the relay client) dials the union the ROOT behaviour's pending hook returns — the Swarm calls it once — so the funnel is a wrapper AROUND the composed behaviour pruning that union by class before any socket (rule 5); a sibling hook sees only the explicit list. Rule 8 in two halves: STORES (book, routing stash, PeerCache re-entry, AutoNAT servers, relay reservations) hook at their learn sites; a behaviour-EXTENDED address is enforced at the root and nowhere else, while a store dialled EXPLICITLY (AutoNAT learned servers, relay reservations) is enforced by its learn-site hook, since the funnel passes the explicit list untouched. Rule 9: provenance is the DOOR — one operator set, consulted at every door and at the root, admits the operator's names; no carrier gains a provenance field. Identify's crate cache disabled; the funnel, its measurement (the union reaches the outer wrapper; the test is the record, no spike log), the operator set seeded from the profile, and the AutoNAT/relay/query-candidate store hooks all LANDED on PR #111 (held by the owner for the full closure); plan §15's precondition reads MET on that head. Still owed for Stage 12: a peer-door learn command — `add_address` is the operator door and a composer must not pipe discovery output through it. Rule 2 names `AddAddress`.
- A peer's own circuit address enters the book through the boundary (Amendment 2026-09-25): rule 2's no-circuit clause is the dial-back's and the punch's; the Identify instance admits an advertised `/p2p-circuit` whose transport prefix through the relay's `/p2p` component meets the floor or is the operator's — a NATed peer's only route — with the relay's admission the gate's at dial time; mDNS keeps the unscoped floor.
- Keywords: address class, SSRF boundary, peer-supplied address, dial-back target, hole punch candidates, loopback, link-local, RFC 1918, ULA, pending outbound hook, is_probeable_address, resolver policy, peer-supplied name, identify listen_addrs, learn site, root funnel, advertised circuit, extend_addresses_through_behaviour, store versus dial, operator set, provenance door, AddAddress, identify cache

...
Infrastructure Identify auto-candidate flags default **false**. Static configured relays are preferred; static AutoNAT servers are guaranteed to be DIALLED rather than preferred among those already connected (`AUTONAT.md` §3, Amendment 2026-09-09). Explicit opt-in to Identify-learned authorized infrastructure is a topology convenience, never trust promotion.

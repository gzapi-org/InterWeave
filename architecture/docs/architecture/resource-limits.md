...
| mDNS crate per-interface send buffer (ADR-0053 D2, D4) | 16 packets (`MAX_INTERFACE_SEND_PACKETS`); one reply per second per interface (`MIN_ANSWER_INTERVAL`, RFC 6762 §6) | fixed; excess queries counted, not answered |

...
| DCUtR attempts in flight | 4 | 32 | the relayed connection gets no DCUtR handler (`declined_busy`); the attempt is the unit, not the dial — one punch is a dial at both ends |
| DCUtR attempts per peer | 1 | 4 | the same (`declined_peer_busy`); a failed attempt puts the peer in the five-minute cooldown (`declined_cooldown`), a direct connection already open declines too (`declined_direct_exists`) |

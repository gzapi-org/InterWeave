...
Every dial these behaviours cause crosses `DialAdmissionGate` under an attributable origin (`autonat-probe`, `relay-reservation`, `relay-circuit`, `dcutr-hole-punch`) — but they are not all announced the same way. `relay-reservation` and `dcutr-hole-punch` are behaviour-originated: the behaviour emits the dial and a wrapper around it announces the origin. `autonat-probe` splits by ROLE — the v2 SERVER's dial-back is behaviour-originated, while the CLIENT emits no dial at all (a probe is a request over an already-open connection), so on the client side that origin belongs to this crate's own `attempt_dial` toward a configured server. Only Kademlia is wrapped today. The AutoNAT CLIENT is constructed when `SubstrateConfig.autonat_client` is `Some` (default `None`) and is deliberately NOT wrapped — it emits no dial, so there is nothing to announce; its static servers are dialled by its driver through `attempt_dial` under `autonat-probe`. The relay client, the relay server, the AutoNAT server and DCUtR are compiled since Stage 11's features-on change but constructed by nothing, so for those this is the shape the stage builds, not one it has. **`relay-circuit` is a command-path origin.** SPIKE-004 measured that on the pinned `libp2p`, `relay::client::Behaviour` emits no dial for a `/p2p-circuit` address — the relay TRANSPORT handles that address — so no behaviour wrapper sees the dial toward the destination. It does emit two dials of its own, both naming the RELAY: one for a reservation and one to establish the relay connection a circuit request needs. Both are exchanges *with* the relay and take a reachability origin — the side of the split `DialOrigin::names_application_destination` answers `false` for. **Labelling that second one `relay-circuit` would break relaying**: that origin names an application destination, so the set-up dial toward an infrastructure-only relay would be refused, for exactly the peers relaying exists to reach. The caller supplies the origin instead: `attempt_dial` builds the `DialRequest` carrying it and admission runs there, and `AdmittedDial::from_ticket` pairs the admitted origin against the address before `GatedSwarm::dial` is handed anything at all. The attribution layer is a per-dial classifier rather than one fixed origin per wrapper precisely so a later crate version that DOES emit a circuit dial from the behaviour is answerable without redesign. Attribution built only into a behaviour wrapper would let circuit dials reach the gate under the wrong origin. Infrastructure-only PeerIds are admitted only for the two reachability origins — `autonat-probe` and `relay-reservation`. `relay-circuit` and `dcutr-hole-punch` **name an application destination, and stay so regardless of what the destination turns out to be**: each names the far end of an application path, so one toward an infrastructure-only peer is refused (for `relay-circuit` by ADR-0036's Amendment 2026-09-03; for `dcutr-hole-punch` by the matrix row that has always been there), and one toward a trusted peer reached *through* infrastructure is admitted on that trusted destination's class. What varies is the destination's class, never what the origin names. **This is the shipped gate as of Stage 11 step 2 (2026-09-04)**, not merely the target: the predicate now names both origins and is called `DialOrigin::names_application_destination`, so a circuit or hole punch toward an infrastructure-only peer is refused. Until then it omitted both and admitted them — SPIKE-004's divergences D2 and D1.

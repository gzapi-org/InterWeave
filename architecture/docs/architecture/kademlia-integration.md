...
**The address-class boundary at the routing door (ADR-0052 rule 8; the instance, stated 2026-09-25 for the hook built on PR #111, commit 66e5663).** The routing table is the second store this node dials from, and Identify feeds it the same `listen_addrs` it feeds the address book. Every writer to the stash ahead of `add_address` — Identify's `listen_addrs`, and every `OfferRoutingPeer`, whether a query result or a PeerCache hint on re-entry — passes one predicate at the stash (`admits_offer`), the book's rule at the second door: ADR-0052's floor with rule 2's no-name clause, rule 3 admitting a private address only while this node holds a non-loopback listener of the same family (the listener set is read live before each dispatch, not held from start-up), and no rule-4 clause. A refused address never enters the stash, is counted by class on the driver's state and is never logged. This is a STORE hook in rule 8's terms: it keeps the routing table clean, and it is not the dial's enforcement. What Kademlia DIALS — the routing table's addresses, the in-query FIND_NODE addresses its walk collects, and whatever the other behaviours add through `extend_addresses_through_behaviour` — is enforced once, at the root funnel of ADR-0052 rule 5 (A 2026-09-25), the wrapper around the composed behaviour that lands on the same PR; until it has landed, Stage 12 composes no profile that enables `kademlia` (plan §15). One correction rides with the rule: an operator's own `/dns4` seed from the static-bootstrap provider reaches this door as an `OfferRoutingPeer` carrying peer and addresses only, and the floor as first built refused it — fail-closed, and unreachable before Stage 12; under ADR-0052 rule 9 `admits_offer` consults the operator set, so the operator's seed routes and a peer's name still does not.

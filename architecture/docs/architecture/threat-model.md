...
| Endpoint source spoof by ordinary local caller | local app claims another source route | source endpoint derived from local-session lease; not command input | compromised daemon/runtime | process isolation |
...
| Offline mailbox creep | implementation stores messages for absent endpoint | contract forbids daemon buffering; no Accepted without active queue | human app separately retains only ADR-0044 pending-outbound/unread/receiver-kept content after local application interaction | capability-explicit durable transport backend only |
| Admin confused deputy | network message causes trust/endpoint mutation | platform admin authority is inaccessible to remote handlers: desktop uses a separate admin socket whose data socket categorically denies admin.*; Android uses a distinct `LocalAdminPort`; explicit local gesture | same-process GUI bug/social engineering; malicious same-UID desktop process may open admin socket; arbitrary Android same-process compromise is inside the app trust boundary | SPIKE-005 stronger desktop OS auth/user presence; minimize Android platform bridge authority |
...
| Android mnemonic UI exfiltration | screenshot/recording/Recents/IME/clipboard or telemetry captures the 24-word private-key backup | dedicated non-exported recovery Activity, `excludeFromRecents`, `FLAG_SECURE`, in-app word picker, no clipboard path, no phrase in saved state/logs/analytics/crash artifacts | compromised OS/accessibility/IME or running-process compromise remains outside complete app protection | SPIKE-008/009 device/version matrix; minimize recovery exposure time |
| Android system backup/privacy leak | wrapped identity/config/human-store is copied to cloud/device-transfer or creates a half-restored profile | `allowBackup=false` plus explicit cloud/D2D extraction exclusions for identity/config/recovery/human-store; restore never manufactures a PeerId | OEM/platform backup behavior can change; user may separately export app data outside supported flow | SPIKE-008 restore matrix; future explicit encrypted backup may include only ADR-0044 unread/kept inbound content |
...
|---|---|---|---|
| authorized AutoNAT client uses probe server as SSRF scanner | AutoNAT server | literal candidate IP must equal observed requester source IP; reject loopback/private/ULA/link-local/multicast/special-use; no requester DNS resolution | authorized client can still request permitted ports on its own observed public IP within quotas |
| implicit Identify turns friend into relay/probe | infrastructure selection | Identify candidate flags default false; static candidates preferred; authorization still required | operator opt-in leaks metadata/bandwidth to selected authorized peers |
| circuit flood hides original source IP | destination pre-Noise | bucket by authenticated relay connection/PeerId + global caps; relay source quotas complement | malicious relay can consume its own bucket/global share until blocked |
| Android remote message triggers admin action | mobile local boundary | event handlers receive LocalDataSession only; LocalAdminPort exists only in explicit UI path | arbitrary same-process compromise can call app internals |
| Android key extraction from filesystem | identity at rest | Keystore AES-GCM wrapping, app-private ciphertext, optional user-presence mode | unwrapped Ed25519 seed exists in Rust process memory while online |
| remote sender attempts to force message persistence | human application retention | Keep is receiver-local, available only after read; HumanChatV2 has no persistence request field | compromised human client can violate its own local policy |
| logs/notifications/search indexes become shadow chat archive | local privacy | ADR-0044 deletion applies to application-owned plaintext caches; no message bodies in analytics/logs; notification content bounded/user-controlled | OS/filesystem forensic remnants depend on platform/storage layer |

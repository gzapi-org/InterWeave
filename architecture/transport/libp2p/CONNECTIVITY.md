...
**Where it runs (2026-09-19, step 10).** A network change IS a change in the set of addresses this profile's listeners have bound, compared without the interface-scoped ones — loopback, unspecified, link-local — after the first bind (`runtime/network_change.rs`); a wildcard listener reports each interface's address as it comes and goes, so an OS-level change arrives as the same listener events a `Listen` or `StopListening` command raises, and an interface change that leaves the bound set intact is not seen (binding an OS network monitor is the Android step's). It is detected ONCE, by the runtime, at the listener event that changed the set, and reported; only a change that REMOVED an address invalidates what was known — item 1 speaks of removed addresses, and an interface coming up, a VPN, or a wildcard listener reporting one more of a multi-homed host's addresses at startup says nothing against the addresses still held, so it is offered as a candidate and forgets nothing. A removal is told to every subsystem in the same turn — with the AutoNAT client off as well, which the client's own comparison never covered: (1)–(3) the AutoNAT adapter's `network_changed` sends the verdict to `unknown` and publishes it, withdraws the advertised addresses, restarts the wrapper's listener set with the listeners still bound offered again at once, and prunes the departed candidates on the next tick; (4) the published `ConnectivityChanged` is what the relay target follows, in the same turn; (5) a control connection or a reservation over an address still bound is kept; one over an address in `removed` — matched on the LOCAL endpoint's IP where that IP appears in `removed` and in no address still bound: `removed` holds listener-bound addresses while an outbound relay control connection runs from an ephemeral local port, so a full-multiaddr match would close nothing, and two listeners on one IP (TCP and QUIC, or two ports) losing one keep the IP bound and close nothing either; the LOCAL IP of an OUTBOUND connection is not reported by libp2p (libp2p-tcp binds every dial to the unspecified address), so the runtime records the kernel's route source toward the remote at establishment and matches that, and a connection dialled by NAME records none and is closed on any departure, one reconnect being the cost (Stage 11, #129) — is CLOSED by the runtime at the change — **the rule since 2026-09-26** (SPIKE-004 phase B, node row `ifchange`): as first built the runtime closed nothing and trusted a connection that died with its interface to "close on its own", and measured, for the 120 s after an interface removal and the 120 s after reconnection on a new address, the client lost and rebuilt no reservation, still counted two peers, and neither relay saw its connection end — a connection whose local address is gone cannot be functional, and item 5's "if the OS permits" was never meant to keep it; the closed connection takes the ladder with its jitter (§8). A path that dies WITHOUT an interface event — a NAT rebinding, a carrier drop — is covered by the second rule of the same day: **relay control connections carry a keepalive** (the owner, 2026-09-26, "keepalive yes", relayed by p2p-network-dev-01 in GZCoord `01a0dda0-5b20-7217-a603-815dbbb74e1f`; its traffic and battery cost is the accepted price of a client not believing itself reachable through a relay it cannot reach) — the bounded ping §4's matrix already admits on infrastructure connections, at the crate's default interval, on the connections of relay control PEERS only; a missed liveness closes the connection and takes the ladder exactly as a removal does. As built (Stage 11, #129) the UNIT is the relay control PEER, not a connection — libp2p-relay 0.22.0 names no connection for a reservation outside the crate (both public `ReservationReqAccepted` events carry a PeerId and no ConnectionId) — so every connection to such a peer is pinged, whatever its class, and a missed liveness (two failures in a row) closes only the connection whose own ping failed; the ping runs at BOTH ends: the client pings the relay it holds an active reservation on, and the relay pings each peer holding, or having held until its last connection closes, one of its reservations — without the relay's ping a rebuild after an interface change stalls, the relay keeping the dead reservation and refusing the client's new one for per-peer room until it lapses — and every end ANSWERS pings on infrastructure connections, so `ping` is in a relay-configured profile's protocol set there. Both rules were built by p2p-network-dev in one PR with RELAY.md §8's hop gate (#129); (6) every reachability candidate is due for a re-test within one crate tick of jitter (`NETWORK_CHANGE_JITTER_MS`), drawn apart; (7) nothing is replayed — the runtime holds no frame to replay, and an exchange the transition failed was answered to its caller. DCUtR state is rebuilt too (`contracts/CONNECTIVITY.md`): every attempt in flight is given up — it keeps its per-peer permit while the crate's rounds on the kept relayed connection run, and ends `Abandoned` with no cooldown whatever then reaches it, a landed punch excepted, which is `Succeeded` — every cooldown is lifted, and a punched connection in its interval stops being judged. The consumer is told as `NetworkChanged { removed, added }`. `tests/connectivity/tests/dcutr.rs` pins the report, an addition leaving the cooldown standing, a removal lifting it, the kept reservation and a given-up attempt's kept permit over real sockets; the AutoNAT half is the adapter's unit test, since loopback yields no evidence to invalidate.

...
**Note (2026-09-18, step 5).** Two things the pinned client does that the rules below do not say. A `Release` withdraws the address and removes the listener, but the client keeps the reservation alive on the relay: the handler holds it, keeps the connection alive and renews it, until a message to the removed listener fails — the next renewal (the crate's default lifetime, an hour) or the next inbound circuit, which is then dropped. So `released` on this side and the relay's per-peer reservation count disagree for up to an hour, and the control connection stays open that long; closing it was left, on this date, to the step that owns relay connection lifetime (step 7, beside the AutoNAT adapter's idle-churn question) -- not a knob the client has. **Taken 2026-09-19 (step-5 follow-up, the owner's direction):** at every release path -- the reconcile tick, the relay event arm and `follow_verdict` -- the runtime closes the control connections whose origin is `RelayReservation` (`released_control_connections`, `runtime/relay_driver.rs`); a relay v2 reservation is bound to the connection that asked for it, so closing that connection returns the slot. Only that origin is closed: a relay may also be a data-plane peer, and a reservation ending is no reason to drop an application connection to it; an inbound is recorded origin-less and is never closed (mutation-checked: without the origin test the fixture's three connections are all selected instead of one). **Owed, and not constructible on this host:** that the relay's per-peer count observably returns on a SURPLUS release — the other two release paths close the connection anyway (a de-authorisation is a revocation, a lost relay has no connection left), so the path this changes needs a real direct-inbound verdict, and a real verdict needs a real AutoNAT probe of a probeable address: `is_probeable_address` (`crates/transport/runtime/src/reachability.rs`) accepts only a public `/ip4` or `/ip6` literal, and this host has loopback and one RFC1918 address, so no probe is made and no verdict arrives at any harness size (measured 2026-09-19; DCUtR's address-class boundary admits a private pair, AutoNAT's §6 does not, which is the whole difference). The test therefore sits behind the same wall as SPIKE-004 phase B and belongs in its containerised NAT row, or on a host with a public address; until it runs the rule is pinned by the unit test and the observable return is not. And the client's ask carries, beside the configured address, whatever Identify's cache and the Kademlia table hold for the relay (`extend_addresses_through_behaviour`) — a circuit address of the relay among them would be dialled through the relay transport under the same origin; step 7 settled it at the established hook, where a connection that came up over a circuit is judged under `RelayCircuit` whatever dialled it and so refused toward an infrastructure-only relay (`retention_origin` in `runtime/dialing.rs`).

...
**Note (2026-09-25).** The release half of the note above describes libp2p-relay 0.21.1. Since 7459dc98 (2026-09-19) the lockfile pins 0.22.0, where removing the listener reaches the client behaviour as `ListenerClosed`: `on_listener_closed` expires the reservation's external address and sends `ResetReservation` to that connection's handler, which drops its reservation, so `connection_keep_alive` is false and there is no renewal. The control connection then closes at the Swarm's idle timeout unless something else still uses it — a data-plane relay's application traffic, or live circuits — and the relay frees the reservation the moment the connection closes (`on_connection_closed` → `ReservationClosed`). So `released` here and the relay's per-peer count disagree for the idle timeout of an otherwise-unused connection, not for an hour (a data-plane relay's connection may outlive the release; the relay then drops the unrenewed slot at its own `reservation_duration` — an hour by default, `reservation_duration` in this project's server profile — since the connection's handler arms that timer on accept and on renew and expires the reservation when it fires, connection or no connection), and no forced close of the control connection is owed: one that cut a data-plane connection or live circuits would be a defect, which is why PR #115 closed unmerged. Read at 0.22.0 in `priv_client.rs` (`on_listener_closed`), `priv_client/handler.rs` (`In::ResetReservation`, `connection_keep_alive`), `behaviour.rs` (`on_connection_closed`, `ReservationTimedOut`) and `behaviour/handler.rs` (`active_reservation`); not measured on the wire on this host (the surplus path needs a public literal).
...
**Note (2026-09-18, step 6).** The server role is the pinned `relay::Behaviour` under the class gate for the infrastructure service (`runtime/relay_server_driver.rs`): the hop protocol is offered to the two authorized classes and to nobody else, which is the service admission below — together, since 2026-09-26, with the verified-address gate stated at the end of this note. Every ceiling above is set from the profile, none left to the crate — `relay::Config::default()` is 128 reservations, 4 per peer, 16 circuits, 120 s and 128 KiB per circuit, wrong in both directions (SPIKE-004 F10) — and the two per-peer ceilings are handed to the crate ONE BELOW the profile's, because the crate refuses a per-peer request when the count is already greater than its ceiling and so admits one more than it is told; the totals it refuses at equality — and it counts a RENEWAL against the total (the per-peer check is guarded by `!renewed`, the total is not), so a relay AT its reservation ceiling refuses its clients' renewals, sheds every one of them at three quarters of the duration and reopens the slots at the full duration for whoever asks first: size `max_reservations` above the client population, and treat a steady state at the ceiling as a step-7 question no configuration can express. `max_circuits_per_peer` bounds the circuits a PeerId is PARTY to, as source or destination — the crate counts both — which is stricter than "per source". An earlier version of this list carried `max_pending_control 64`, and the profile block a key for it. Neither reached a mechanism: the crate has no field for it (F10), and a wrapper sees a control request in the behaviour's public events only once the behaviour has already answered it. The key was removed as the AutoNAT server's `timeout` was (`AUTONAT.md` §7). What bounds control work is the crate's own per-connection concurrency — at most ten inbound hop streams in flight per connection — times the connection ceiling the root policy holds, and the crate's rate limiters, kept at their defaults — which are TOKEN BUCKETS, not rates (SPIKE-004 phase B, node row `ratelimit`, 2026-09-26; `libp2p-relay` 0.22.0 `behaviour.rs:125-160`, `rate_limiter.rs`): per peer a bucket of 30 refilled one token per two minutes, per IP a bucket of 60 refilled one per minute, for reservations and for circuit sources alike. An earlier version of this sentence read them as "sixty per IP per minute", which is wrong in kind: the recorded run: 120 circuit dials from one address logged 60 accepted and 120 denied `ResourceLimitExceeded` (a dial can ask the relay more than once, so outcomes outnumber dials) — the same status a full table gives — and four minutes later ten fresh dialers from the same address logged 4 accepted, the bucket's refill of one a minute, measured. So a shared address, or a carrier's CGNAT behind which many clients present one IP, gets 60 circuits and then one a minute; the circuit ceiling below is a table SIZE and this is a RATE: 60 circuits per source address at once, then one a minute per address, and with hour-long circuits the ceiling of 128 is reached by three addresses at once or by two in about four minutes (the cost row's extrapolation, about 4.7 MB at 37 KB a circuit and one descriptor per reservation, stands as the budget check). The defaults are KEPT: raising the per-IP bucket for CGNAT is a configuration decision that waits for the deferred carrier-CGNAT measurement, and is recorded here as such rather than guessed. A reservation's addresses are the Swarm's WHOLE external set — the AutoNAT verdict's verified addresses and, for a profile that is also a relay client with an active reservation, the relay-derived circuit addresses too, since the crate sends every external address on accept and offers no filter — so such a profile would hand its clients a nested circuit address the pinned server cannot serve. Step 7 withholds them: the server sits under `ServedAddresses` (`crates/transport/libp2p/src/served_addresses.rs`), which drops a circuit address's confirmation and expiry before the crate learns it, so a reservation this profile grants carries its direct verified addresses only — measured on the wire by `tests/connectivity/tests/relay_server.rs`'s dual-role test (since Stage 11, #129, it pins that the derived address does not open the gate, while the filter itself is proved at the crate surface by `crates/transport/libp2p/tests/relay_hop_gate.rs`: a circuit address confirmed beside a direct one is not handed on), where without the wrapper the client is handed `…/p2p-circuit/p2p/<self>/p2p-circuit`. A relay with no verified address would accept reservations that carry none — the crate serves under a forced `Status::Enable` and has no verdict input — and the client refuses each (`NoAddressesInReservation`) while the relay counts it against its ceiling until it closes: measured on the phase-B matrix (node row `early`, 2026-09-26) at a ceiling of two, eight asks refused for want of room from 7.6 s to 89.8 s and nothing granted between its verification (35.0 s) and its last denial, while the control at a ceiling of three refused nobody; libp2p-relay 0.22.0 reads a client's re-ask over the same connection as a renewal, skips the per-peer ceiling and refuses on the TOTAL, which the phantoms hold. **Rule (2026-09-26): the relay server offers the hop protocol — and so accepts a reservation — only while `ServedAddresses` holds at least one verified direct address.** Before the first AutoNAT verdict, and whenever the served set empties, every reservation request that arrives is refused — new or renewal, on a new connection or one already open — since a relay with no verified address can serve neither. This gate answers PER REQUEST — a wrapping connection handler refuses the inbound hop stream itself (its `listen_protocol` substitutes the denied upgrade), which refuses RESERVE and CONNECT alike and leaves the crate's forced `Status::Enable` untouched; the crate's denial status is `pub(crate)`, so no code above it can send one, and the client sees the hop protocol as unsupported (`ReserveError::Unsupported` / `ConnectError::Unsupported`), not a status — not at connection admission: the class gate decides once when a connection is established and never revisits an admitted one, so it cannot refuse a renewal on an open connection, and closing the open connections instead would drop every reservation at once. The gate is read TWICE: at stream negotiation, and again where the RESERVE or CONNECT reaches the behaviour — the crate reads the request up to 60 s after negotiation and grants from the addresses it holds THEN, so a set that emptied in between must still refuse; a request dropped at arrival ends its stream, which the client sees as an I/O end (`ReserveError::Io` / `ConnectError::Io`), not `Unsupported` (Stage 11, #129; the late drop is asserted by the gate module's comment and `refused_late == 0`, not yet by a test that shuts the gate between negotiation and the request). A client loses its reservation at its next refused renewal (the crate sheds at three quarters of the duration) and its ladder rebuilds elsewhere; the relay's own entry lapses at the full duration. The gate sits one level above the crate, where the served-address filter already sits. No reservation carrying no address is then ever granted, and none holds the ceiling. `Status::Enable` stays forced below it. The gate is built (Stage 11, #129). Its crate-surface test is `crates/transport/libp2p/tests/relay_hop_gate.rs` — shut with no address and with only a circuit address, opened by a direct one; with the gate open the ceilings exact and a stranger offered nothing. `tests/connectivity/tests/relay_server.rs` keeps the production runtime's side: retention, the gate SHUT on loopback (Identify and ping only; RESERVE and CONNECT refused `Unsupported`), the stranger closed at establishment, and the dual-role test against an upstream relay. Before Stage 11 it recorded the address-less acceptance as loopback's limit.

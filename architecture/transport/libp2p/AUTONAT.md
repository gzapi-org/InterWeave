...
**Where it runs (2026-09-19, step 10).** The network change of the first row is the runtime's (`CONNECTIVITY.md` §14's note): a change in the bound listener set, detected once by the runtime and handed to the adapter's `network_changed`, which sends the verdict to `unknown`, publishes it, withdraws the advertised addresses, and returns every tracked candidate to the sweep within one crate tick of jitter — the re-test is what moves the table off `unknown` again, and its jitter is §14's bounded re-probe. Until step 10 the adapter compared the set itself on its tick, so a change was seen only with the client on.

...
**Where it runs (2026-09-19, step 9).** `direct_candidate` is a punched direct connection the runtime's path derivation ranks BELOW the relayed connection until it has held for `direct_stability_period` (`OpenConnection.punched`/`since_ms`, `dialing::PathSample`): the relay stays the announced path, and the move to direct — `PeerPathChanged { relayed → direct, HolePunched }` — is announced from the runtime's tick once the interval has passed, so a punch never emits a second `PeerConnected`. One case is earlier: if the relayed connection closes first — the far end retired it at its own instant, or the relay dropped it — the young punched direct is the only connection left and becomes the announced path at its close, by the punch, since the announced path is always a connection that exists; the interval gates the move only while the relayed connection stands. `direct_preferred`'s arrow is the runtime's retirement: once a stable punched direct is the announced path, the relayed connections to that peer are closed WHEN SAFE — no direct or directory exchange this profile started with the peer awaits its answer — and reported as `RelayedConnectionRetired`; the far end's exchanges on that connection, if any, fail there, and it retires at its own stability instant too; the reservation and the circuit route stay. A punched connection that closes within the interval is a stability failure at the wrapper (`HolePunch { Unstable }`, §8's `direct_upgrade_stability_failures_total`), the peer enters the cooldown, and the relay was never left; one that holds is `direct_upgrade_success_total`. A DIALLED direct connection joining a relayed one (step 7's case) is announced at once — its completed handshake is its evidence, and the interval is the punch's — and the relayed connection behind it is retired the same way, when safe: the retirement reads "a stable direct connection is the announced path", not "the punched one is", because a second direct connection beside the punch (the far end's crate retrying its stalled punch dial after the attempt ended, measured with a bare far end) provides the path over the punched one and would otherwise keep the redundant relayed connection open for as long as the peer is in use, since request-response spreads a peer's streams over every connection to it. `tests/connectivity/tests/dcutr.rs` pins the gate, the retirement and its waiting for an exchange in flight on the wire, over the host's private-range pair, with a two-second interval; `relayed_paths.rs` pins the retirement behind a dialled direct.
...
**Where it runs (2026-09-19, step 10) — a network change.** `CONNECTIVITY.md` §14's change GIVES UP every attempt in flight (its CONNECT carried the old network's addresses, and a failure met now is not the far end's) without removing it: the crate's rounds on the kept relayed connection run on and the wrapper cannot stop a handler, so the attempt keeps its per-peer permit — a second circuit from the peer is `PeerBusy` meanwhile — and ends `Abandoned`, with no cooldown, whatever the crate then reports, at the relayed close, or at the horizon — unless the punch lands after all, which is `Succeeded` and judged for stability like any punch, since a direct connection is evidence about the path whatever network its CONNECT was written on; removed at once, the crate's late outcome would be charged to the peer's next attempt and re-earn the cooldown the change lifted. It lifts every cooldown (a peer that could not be punched from the old network is tried from the new one on its next circuit), and stops judging a punched connection still in its stability interval (its close, if it comes, is the interface's). `HolePunchScope::network_changed`; pinned by its unit test and, on the wire, by `tests/connectivity/tests/dcutr.rs`'s lifted cooldown and kept permit.

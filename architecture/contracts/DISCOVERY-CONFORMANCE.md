...
**Decision 2026-09-25.** Guarantee 5 binds the PROVIDER's state and emitted batches, and those are bounded; one store beneath the provider is not, and this paragraph is the amendment CLAUDE.md §6 requires before an unbounded structure may stand. `libp2p-mdns 0.49.0` keeps its own record store — a `SmallVec` with no cap (`behaviour.rs:141`), searched linearly per record (`:328-331`, so CPU is quadratic in what the multicast domain announces), expiring on the ANNOUNCER's own u32 TTL (`query.rs:184`) — and nothing in this repository can bound it without changing the crate (the mDNS review of PR #111, F2). What bounds the harm: since f85dd27 that store reaches no dial and no book, so its exposure is memory and CPU on this node, from hosts on the same multicast domain only; and the mechanism ships gated off, so a profile that enables `mdns` does so on a multicast domain the operator controls, and `providers/mdns.md` §Security tells the operator what that admits. The exception is time-boxed to the mDNS deadline: the store is BOUNDED before that deadline reads MET (plan §14) — by vendoring the crate under ADR-0051's route with a cap and eviction, or by a wrapper bound that replaces the inner behaviour at a cap and re-discovers; p2p-network-dev chooses the mechanism and states it in `providers/mdns.md`. Adversarial coverage is owed with the exception, not after it: a flood of N distinct announcements measured for memory and time, committed as a test, so the growth is a number and not a sentence.

**Widened 2026-09-25, later the same day.** The paragraph above names one uncapped structure; the crate has three, read in the pinned source: the record store, and per interface a buffer of discovered pairs that grows whenever the behaviour is slow to drain it and a send buffer that every received query extends with a response packet — the last an amplification path, since each entry is multicast onto the LAN. The exception covers all three under the same terms. Its closure is decided: ADR-0053 vendors the crate with caps on the three, the announcer's TTL clamped to the provider's, RFC 6762's once-per-second reply rule and an interface-failure event; the flood harness that measures the unpatched crate is that record's first step, and the deadline above reads MET only when its rule 9 is met.

...
**What produces the degraded signal (decided 2026-09-25).** Today only a failed interface watcher does (`SwarmEvent::MdnsUnavailable`, emitted once, before any other event). `libp2p-mdns 0.49.0` logs a failed multicast bind, join, send or receive inside the crate and emits nothing, so a network that blocks multicast is indistinguishable from a LAN with no other peer — silence is never the signal, and the absence of discoveries MUST NOT be reported as degradation. The signal comes from one of two mechanisms, and the one chosen to bound the crate's store (§Security) decides which: a vendored crate (ADR-0051's route) reports those failures as events, which the wrapper maps onto degraded; a wrapper-bounded crate needs a self-hearing probe — the driver sends a query and expects its own announcement back within a window, and a node that cannot hear itself is on a domain that does not carry multicast, which is what SPIKE-010's environment probe measures from outside today. Either way the degraded state is evidence-backed and the row in `SPIKES.md` runs against it; owed before the mDNS deadline reads MET (plan §14), p2p-network-dev's mechanism.

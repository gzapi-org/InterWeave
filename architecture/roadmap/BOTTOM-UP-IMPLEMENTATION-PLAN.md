...
- connectivity-infrastructure peers never gain GossipSub/direct/endpoint/Kademlia authority merely by being connected, and the protocol set such a connection is OFFERED is restricted at the connection rather than answered at the request. **MET by `ClassGated<B>`** (`crates/transport/libp2p/src/class_gate.rs`).
  ...
  **What it does not do, and what covers the difference.** A handler is built once at establishment and never rebuilt, so a peer PROMOTED while connected stays gated until it reconnects. The opposite direction is what matters for safety and is handled elsewhere: revocation closes the connections it downgrades, and `sync_broadcast_admission` blacklists a downgraded peer from the mesh — which rejects its MESSAGES while leaving `/meshsub/` registered, so that call is authority and this wrapper is exposure, and neither substitutes for the other.

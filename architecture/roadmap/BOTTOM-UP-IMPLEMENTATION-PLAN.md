...
- **`mdns` — a deadline this stage was given and has not yet met:
  ...
  **One more condition of MET (2026-09-25):** three structures in the
  crate are unbounded and one is announcer-TTL'd — the record store and
  two per-interface buffers, one of them a query-amplification path
  (`DISCOVERY-CONFORMANCE.md`, Decision 2026-09-25 and its widening);
  they reach no dial and no book since f85dd27, so the exception stands
  with a flood measurement, and the deadline reads MET only once
  ADR-0053's rule 9 is met: the crate vendored and capped under
  ADR-0051's route, the announcer's TTL clamped, RFC 6762's
  once-per-second reply rule, the interface-failure event, and the
  flood harness asserting all of it — the work the owner ordered
  started on 2026-09-25, p2p-network-dev's.

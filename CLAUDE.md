...
- Stage 0 is **complete** and **Stage 1 is open**. The root Cargo workspace has three members: `xtask` (the command runner), `tests/support` (the test-only harness), and `crates/api/transport-api` (the first neutral contract — types and validation, no I/O). The first two are not product code. No further package joins until the stage that needs it opens (§3).

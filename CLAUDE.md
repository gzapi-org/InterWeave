...
- Stage 9 is open and activates the first new packages since Stage 3 — `crates/discovery/{static,cache,mdns}` — added one at a time as each provider is built (§3), so `[workspace].members` grows again here. Stage 8 activated no new package: `/interweave/endpoints/1.0.0` landed in `crates/transport/libp2p` with its tests under the existing `tests/endpoint-routing`. The Stage-1 contract crates under `crates/api/` remain types and validation only — no I/O, no runtime, no backend — and the Stage-2 crates remain pure state machines.

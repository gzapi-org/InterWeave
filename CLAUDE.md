...
- **Stage 10's `Met.` block records one clause met by SCOPE ONLY**, and it is the one a reader is most likely to act on: "configured entries default on" has no test and cannot get one here. There is no composition root to hold a default (Stage 12), and SPIKE-003 separately withholds shipping default-enabled until SPIKE-004 supplies the server-mode reachability evidence ADR-0034 requires. A later stage AND a later spike; the stage closing cleared neither. The block also records that the only place the port's two halves have ever run against each other is `tests/kademlia/tests/overlay_health.rs`, and what that test does not prove.

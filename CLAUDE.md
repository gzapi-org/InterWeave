...
- Stages 0-5 are **complete** and **Stage 6 is open**. Read `[workspace].members` for the current list rather than trusting a roster written here, and `[workspace.metadata.interweave].status` for the open stage. The Stage-1 contract crates under `crates/api/` remain types and validation only — no I/O, no runtime, no backend — and the Stage-2 crates remain pure state machines. Stage 3 landed the first file I/O and the Ed25519 identity; Stage 4 landed the libp2p substrate — TCP, Noise, Yamux, Identify — with every other behaviour left out of the feature list rather than merely unused. Stage 5 builds the root connection and dial-admission funnel, and activates no new package beyond `tests/connectivity`: it is the gate that §3's hard sequencing rule requires before Kademlia, AutoNAT, Relay or DCUtR exist at all. Its implement list and all four exit-gate clauses are met — see the **Met.** block under Stage 5 in the canonical plan for what proves each one. Stage 6 is open: SPIKE-002, its prerequisite, was run and closed **PASS** (`spikes/spike-002/`), and this stage builds `/interweave/direct/2.0.0` end-to-end between real Rust peers, routed to an in-process `LocalDataSession` — no IPC, no UI. It activates exactly two packages, `tests/direct-v2` and `tests/endpoint-routing`, and inherits four findings from the spike that constrain the design before any code exists. No further package joins until the stage that needs it opens (§3).

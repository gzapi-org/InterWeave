...
- Stages 0-2 are **complete** and **Stage 3 is open**. Read `[workspace].members` for the current list rather than trusting a roster written here, and `[workspace.metadata.interweave].status` for the open stage. The Stage-1 contract crates under `crates/api/` remain types and validation only — no I/O, no runtime, no backend — and the Stage-2 crates remain pure state machines. Stage 3 is where the first file I/O lands: the human store, the peer cache, and the profile path layout. No further package joins until the stage that needs it opens (§3).

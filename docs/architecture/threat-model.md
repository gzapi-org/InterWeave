...
| Android system backup/privacy leak | wrapped identity/config/history is copied to cloud/device-transfer or creates a half-restored profile | `allowBackup=false` plus explicit cloud/D2D extraction exclusions for identity/config/recovery/history; restore never manufactures a PeerId | OEM/platform backup behavior can change; user may separately export app data outside supported flow | SPIKE-008 restore matrix; future explicit encrypted history-sync ADR only |
